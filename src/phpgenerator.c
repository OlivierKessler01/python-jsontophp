 ...
 */
char* generate_php_class(struct php_class* php_class) {
 ...
 */
int map_json_to_php_classes(char *json, size_t size_json, struct php_class* php_classes) {
...
char* jsontophp(char *json, size_t size_json, char *conversion_result) {
	...
	if(size_json > 1000) {
		sprintf(result_string, "JSON input is too long, max number of characters is %d", MAX_JSON_CHARACTERS);
	...
	for(int i = 0; i < number_of_classes; i++) {
		strcat(result_string, generate_php_class(&php_classes[i]));
	}
...
int main(){
	...
	char json[1000] = "{class : 'testclass'}";
